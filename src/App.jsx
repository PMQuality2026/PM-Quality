...
const SEED_CLIENTS = [
  {
    id: 1, name: "Ultra Medical Solutions", ref: "PMQ-2026-UMS-001",
    service: "ISO 13485 + SAHPRA Manufacturer Licence",
    phase: "Proposal Submitted", nextAction: "Follow up on proposal decision",
    deadline: "2026-07-05", owner: "Pabi", rag: "amber",
    notes: "Proposal at R97k submitted. Awaiting response.",
    hours: 0, budget: 97000,
  },
  {
    id: 2, name: "GMA", ref: "PMQ-2026-GMA-001",
    service: "ISO 13485 Compliance",
    phase: "Financial Assessment", nextAction: "Await financial assessment outcome",
    deadline: "2026-07-10", owner: "Pabi", rag: "amber",
    notes: "Holding firm on pricing. Phasing/payment sequencing open for discussion.",
    hours: 0, budget: 0,
  },
  {
    id: 3, name: "Mr First Aid", ref: "PMQ-2026-MFA-001",
    service: "ISO 13485 QMS Retainer",
    phase: "Active — Implementation", nextAction: "Month 1 retainer kickoff",
    deadline: "2026-07-01", owner: "Pabi", rag: "green",
    notes: "R22,800/month retainer. Start date 1 July 2026. 24 hrs/month.",
    hours: 0, budget: 273600,
  },
  {
    id: 4, name: "Jalo Enterprises", ref: "PMQ-2026-JAL-001",
    service: "Full ISO 13485 + SAHPRA Manufacturer Licence (Diasol)",
    phase: "Active — Planning", nextAction: "Confirm legal entity registration timeline",
    deadline: "2026-07-15", owner: "Pabi", rag: "green",
    notes: "21-month parallel-track roadmap. New entity, fresh cert required.",
    hours: 0, budget: 0,
  },
  {
    id: 5, name: "Mayomann Integrated Healthcare", ref: "PMQ-2026-MAY-001",
    service: "ISO 13485 + ISO 14001",
    phase: "Proposal Submitted", nextAction: "Follow up on proposal",
    deadline: "2026-07-08", owner: "Pabi", rag: "red",
    notes: "Cape Town distributor expanding into manufacturing. Dual scope.",
    hours: 0, budget: 0,
  },
  {
    id: 6, name: "LinkedIn AR Enquiry", ref: "PMQ-2026-AR-001",
    service: "SAHPRA Authorised Representative",
    phase: "Lead — Qualification", nextAction: "Send AR service overview and rate card",
    deadline: "2026-06-28", owner: "Pabi", rag: "red",
    notes: "Inbound LinkedIn enquiry. Foreign manufacturer, needs AR service.",
    hours: 0, budget: 0,
  },
];
...
const SERVICES = [
  "ISO 13485 QMS Retainer",
  "ISO 13485 + SAHPRA Manufacturer Licence",
  "ISO 13485 + ISO 14001",
  "SAHPRA Authorised Representative",
  "SAHPRA Distributor Licence",
...
function PhaseTag({ phase }) {
  const isActive = phase.startsWith("Active");
  const isClosed = phase.startsWith("Closed");
  const bg = isActive ? "#E6F4F1" : isClosed ? "#EDF2F7" : "#EBF4FF";
  const color = isActive ? C.teal : isClosed ? C.slate : C.navyMid;
  return (
    <span style={{
...
function Modal({ client, onClose, onSave }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: "#fff", borderRadius: 12, padding: 32, width: "min(600px, 95vw)",
        ...
        {[
          ...
          ["Reference No.", "ref", "text"],
          ["Next Action", "nextAction", "text"],
          ["Deadline", "deadline", "date"],
          ["Hours Logged", "hours", "number"],
          ["Budget (R)", "budget", "number"],
        ].map(([label, key, type]) => (
        ...
        {[
          ["Service Line", "service", SERVICES],
          ["Phase", "phase", PHASES],
          ["RAG Status", "rag", ["green", "amber", "red"]],
        ].map(([label, key, opts]) => (
          <div key={key} style={{ marginBottom: 16 }}>
            <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: C.slate, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 5 }}>{label}</label>
        ...
        <button onClick={() => onSave(form)} style={{
          width: "100%", padding: "12px", background: C.teal, color: "#fff",
...
export default function PMQualityOS() {
  ...
  const [view, setView] = useState("dashboard"); // dashboard | tracker | pipeline
  ...
  // Actions due today / overdue
  const actionsDue = clients
    .filter(c => !c.phase.startsWith("Closed"))
    .filter(c => daysUntil(c.deadline) <= 3)
    .sort((a, b) => new Date(a.deadline) - new Date(b.deadline));

  const filtered = clients.filter(c => {
    if (filterRag !== "all" && c.rag !== filterRag) return false;
  ...
  function saveClient(form) {
    if (form.id) {
      ...
      setClients(cs => [...cs, { ...form, id: Date.now() }]);
    }
    setEditing(null);
  }

  function deleteClient(id) {
    if (window.confirm("Remove this client from the tracker?")) {
  ...
  const navBtn = (key, label) => (
    <button onClick={() => setView(key)} style={{
      ...
      background: view === key ? C.teal : "transparent",
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.cream, fontFamily: "'Inter', system-ui, sans-serif" }}>
                ...
                return (
                  <div key={c.id} style={{
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 12, color: C.slate, marginTop: 2 }}>→ {c.nextAction}</div>
                    </div>
              ...
              {clients.filter(c => !c.phase.startsWith("Closed")).map(c => (
                <div key={c.id} style={{
                  ...
                  <button onClick={() => setEditing(c)} style={{
                    padding: "5px 12px", background: C.navyMid, color: "#fff",
        ...
        {view === "tracker" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                <select value={filterRag} onChange={e => setFilterRag(e.target.value)}
                  style={{ padding: "7px 12px", border: `1px solid ${C.border}`, borderRadius: 6, fontSize: 12, color: C.navy, background: "#fff" }}>
                ...
                <button onClick={() => setEditing({ name: "", ref: "", service: SERVICES[0], phase: PHASES[0], nextAction: "", deadline: "", owner: "Pabi", rag: "amber", notes: "", hours: 0, budget: 0 })}
            ...
            <div style={{ display: "grid", gap: 14 }}>
              {filtered.map(c => {
                ...
                return (
                  <div key={c.id} style={{
                    background: "#fff", border: `1px solid ${C.border}`,
                    borderRadius: 10, padding: "18px 22px",
                    borderLeft: `4px solid ${c.rag === "red" ? C.red : c.rag === "green" ? C.teal : C.amber}`,
                  }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10 }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ marginTop: 10, padding: "10px 14px", background: "#F7FBFA", borderRadius: 6, border: `1px solid ${C.border}` }}>
                          ...
                          <span style={{ fontSize: 13, color: C.navy, fontWeight: 600 }}>{c.nextAction}</span>
                        </div>
                      ...
                      <div style={{ textAlign: "right", flexShrink: 0 }}>
                        ...
                        <div style={{ fontWeight: 700, color: days <= 0 ? C.red : days <= 3 ? C.amber : C.navy, fontSize: 13 }}>
                          {c.deadline ? fmtDate(c.deadline) : "—"}
                        </div>
                        <div style={{ fontSize: 11, color: C.slate, marginTop: 1 }}>
                          {c.deadline ? (days <= 0 ? "Overdue" : `${days}d remaining`) : ""}
                        </div>
                        {c.budget > 0 && (
                          <div style={{ marginTop: 8, fontSize: 12, color: C.teal, fontWeight: 700 }}>{fmtRand(c.budget)}</div>
                        )}
              ...
              {filtered.length === 0 && (
                <div style={{ textAlign: "center", padding: 40, color: C.slate }}>No clients match this filter.</div>
              )}
            </div>
          </div>
        )}
                    ...
                    <div style={{ padding: 10, minHeight: 60 }}>
                      {group.length === 0 ? (
                        ...
                        <div key={c.id} onClick={() => { setEditing(c); }}
                          style={{
                            padding: "10px 12px", borderRadius: 7, marginBottom: 8, cursor: "pointer",
                            border: `1px solid ${c.rag === "red" ? C.red : c.rag === "green" ? C.teal : C.amber}`,
                            background: c.rag === "red" ? C.redBg : c.rag === "green" ? C.greenBg : C.amberBg,
                          }}>
                          <div style={{ fontWeight: 700, fontSize: 12, color: C.navy }}>{c.name}</div>
                          <div style={{ fontSize: 11, color: C.slate, marginTop: 2 }}>{c.nextAction}</div>
      ...
      {editing && (
        <Modal client={editing} onClose={() => setEditing(null)} onSave={saveClient} />
      )}
    </div>
  );
}
